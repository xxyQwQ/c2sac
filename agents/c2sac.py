...
class C2SACActor(nn.Module):
    ...
    def log_prob(self, task, state, action, repeat=None):
        ...
        # Keep offline actions away from tanh boundaries so inverse-tanh stays finite.
        action = action.clamp(min=-1 + 1e-6, max=1 - 1e-6)
        return dist.log_prob(action).sum(dim=-1)


class C2SACCritic(nn.Module):
    def __init__(self, num_tasks, state_dims, action_dims, num_layers=2, hidden_dims=256):
        super().__init__()
        self.embedding = nn.Embedding(num_tasks, state_dims)
...
class C2SACAgent:
    def __init__(
        ...
        **kwargs
        ...
        self.actor = C2SACActor(num_tasks, state_dims, action_dims, num_layers, hidden_dims)
        self.actor_optimizer = Adam(self.actor.parameters(), lr=actor_lr)

        self.critic_1 = C2SACCritic(num_tasks, state_dims, action_dims, num_layers, hidden_dims)
        self.critic_2 = C2SACCritic(num_tasks, state_dims, action_dims, num_layers, hidden_dims)
    ...
    def compute_value(self, task, state, action):
        task = self._task(task, state.unsqueeze(0))
        with torch.no_grad():
            value_1 = self.critic_1(task, state.unsqueeze(0), action.unsqueeze(0))
            value_2 = self.critic_2(task, state.unsqueeze(0), action.unsqueeze(0))
